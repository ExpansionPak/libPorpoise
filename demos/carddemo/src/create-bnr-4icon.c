/*---------------------------------------------------------------------------*
  ...
  $Log: /Dolphin/build/demos/carddemo/src/create-bnr-4icon.c $
...
/*---------------------------------------------------------------------------*
   create-bnr-4icon
	 A simple program that creates a memory card file with a banner & 4 icons
 *---------------------------------------------------------------------------*/
...
static void CreateFile(s32 chan, u32 secSize, u32 blockSize);
...
void main()
{
	...
	CreateFile(CARD_CHAN, sectorSize, FILE_BLOCK_SIZE); 
...
/*---------------------------------------------------------------------------*
	...
	Description:    Loads a RGB5A3 icon & banner information into memcard file
	...
	Returns:        none
 *---------------------------------------------------------------------------*/
static void LoadIconRGB5A3( void* CARDbuffer, CARDStat* stat )
{
	...
	// Load the TPL file
	TEXGetPalette(&tplIcons,   "/carddemo/yoshi.tpl");
	...
	CARDSetIconFormat    (stat, 4, CARD_STAT_ICON_NONE);
	CARDSetIconSpeed     (stat, 4, CARD_STAT_SPEED_END);

	// Free the TexPalette
	TEXReleasePalette(&tplIcons);

	// Load the banner TPL file
	TEXGetPalette(&tplIcons,   "/carddemo/gamecube.tpl");
	tdpIcons = TEXGet(tplIcons, (u32) 0);

	memcpy(HeaderRGB5A3.Banner, tdpIcons->textureHeader->data, CARD_BANNER_SIZE);
	// Free the TexPalette
	TEXReleasePalette(&tplIcons);

	// Copy the icon to the buffer
	memcpy(CARDbuffer, &HeaderRGB5A3, sizeof(HeaderRGB5A3));
}
...
/*---------------------------------------------------------------------------*
	...
	Returns:        none
 *---------------------------------------------------------------------------*/
void CreateFile(s32 chan, u32 secSize, u32 blockSize)
{
	s32 result;
	CARDStat stat;
