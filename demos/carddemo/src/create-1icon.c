/*---------------------------------------------------------------------------*
  ...
  $Log: /Dolphin/build/demos/carddemo/src/create-1icon.c $
    
    2     7/02/01 11:31a Dante
...
/*---------------------------------------------------------------------------*
   create-1icon
	 A simple program that creates a memory card file with 1 icon
 *---------------------------------------------------------------------------*/
...
void main()
{
	...
	CreateFile(CARD_CHAN, sectorSize, FILE_BLOCK_SIZE, CARD_STAT_ICON_RGB5A3);
...
static void LoadIconC8( void* buffer, CARDStat* stat )
{
	...
	// Load the TPL file
	TEXGetPalette(&tplIcons,   "/carddemo/marioCI.tpl");
	tdpIcons = TEXGet(tplIcons, (u32) 0);

	// Copy the Texel and CLUT to the structure
	memcpy(HeaderC8.Icon, tdpIcons->textureHeader->data, ICON_CI_SIZE);
...
static void LoadIconRGB5A3( void* buffer, CARDStat* stat )
{
	...
	// Load the TPL file
	TEXGetPalette(&tplIcons,   "/carddemo/luigi.tpl");
	tdpIcons = TEXGet(tplIcons, (u32) 0);

	// Copy icon texel data to the structure
	memcpy(HeaderRGB5A3.Icon, tdpIcons->textureHeader->data, ICON_RGB5A3_SIZE);
