/*---------------------------------------------------------------------------*
  ...
  $Log: /Dolphin/build/tests/arc/src/arctest1.c $
...
void chdirOpenAllocRead(char* dirName, char* pathName)
{
    ...
    if (dirName != (char*)NULL)
    {
        ...
        OSReport("Changing dir to %s ", dirName);
    ...
    if (UseArc)
    {
        ...
        buf = (char*)ARCGetStartAddrInMem(&afinfo);
    }
    else
    {
        ...
        if (OSRoundUp32B(length) != DVDRead(&finfo, (void*)buf, (s32)OSRoundUp32B(length), 0))
        {
