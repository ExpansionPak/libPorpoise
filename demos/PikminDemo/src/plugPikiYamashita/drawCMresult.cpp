...
void zen::DrawCMresult::createRankInEffect()
{
	if (_8C >= 0) {
		...
		Vector3f resultPos(pane->getPosH() + (pane->getWidth() >> 1), 480 - (pane->getPosV() + (pane->getHeight() >> 1)), 0.0f);
		mEffectMgr2D->create(EFF2D_Unk14, resultPos, nullptr, nullptr);
		mEffectMgr2D->create(EFF2D_Unk13, resultPos, nullptr, nullptr);
		mEffectMgr2D->create(EFF2D_Unk12, resultPos, nullptr, nullptr);
		...
		Vector3f scorePos(x, 480 - y, 0.0f);
		mEffectMgr2D->create(EFF2D_Unk16, scorePos, nullptr, nullptr);
		mEffectMgr2D->create(EFF2D_Unk15, scorePos, nullptr, nullptr);
