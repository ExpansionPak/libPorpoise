...
zen::EffectMgr2D* WMeffMgr = nullptr;
...
public:
	...
	void init()
	{
		...
		Vector3f sparkleOrigin(0.0f, 0.0f, 0.0f);
		mSparkleGenerator = WMeffMgr->create(EFF2D_MapOnyonSparkle, sparkleOrigin, nullptr, nullptr);
	}
...
public:
	...
	void init()
	{
		...
		Vector3f exhaustOrigin(0.0f, 0.0f, 0.0f);
		mExhaustGenerators[0] = WMeffMgr->create(EFF2D_MapRocketFire, exhaustOrigin, nullptr, nullptr);
		mExhaustGenerators[0]->invisible();
		mFireFreqFrame = mExhaustGenerators[0]->getFreqFrm();
		mFireInitVel   = mExhaustGenerators[0]->getInitVel();

		Vector3f smokeOrigin(0.0f, 0.0f, 0.0f);
		mExhaustGenerators[1] = WMeffMgr->create(EFF2D_MapRocketSmoke, smokeOrigin, nullptr, nullptr);
...
protected:
	...
	void effect()
	{
		...
		if (mExhaustGenerators[0]) {
			...
			Vector3f exhaustDir(-NMathF::cos(angle), NMathF::sin(angle), 0.0f);
			mExhaustGenerators[0]->setEmitDir(exhaustDir);
		...
		if (mExhaustGenerators[1]) {
			...
			Vector3f smokeDir(-NMathF::cos(angle), NMathF::sin(angle), 0.0f);
			mExhaustGenerators[1]->setEmitDir(smokeDir);
