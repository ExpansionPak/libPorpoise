...
bool zen::DrawHiScore::update(Controller* controller)
{
	...
	case MODE_Operation:
		if (controller->keyClick(KBBTN_B)) {
			...
			mMode = MODE_FadeOut;
			Colour startColour(0, 0, 0, 255);
			Colour endColour(0, 0, 0, 255);
			mAlphaWipe.setColour(startColour, endColour);
...
void zen::DrawHiScore::draw(Graphics& gfx)
{
	...
	if (check) {
		Matrix4f mtx;
		RectArea ortho(0, 0, gfx.mScreenWidth, gfx.mScreenHeight);
		gfx.setOrthogonal(mtx.mMtx, ortho);
		Colour fillColour(0, 0, 0, 255);
		gfx.setColour(fillColour, true);
		Colour auxColour(0, 0, 64, 255);
		gfx.setAuxColour(auxColour);
		gfx.fillRectangle(ortho);
		mScreen->draw();
	}
