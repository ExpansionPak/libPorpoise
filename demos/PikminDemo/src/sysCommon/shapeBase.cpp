...
void BaseShape::makeInstance(ShapeDynMaterials& dynMats, int p2)
{
	...
	if (joint->mChild) {
		Delegate2<BaseShape, Joint*, u32> delegate(this, &BaseShape::countMaterials);
		recTraverseMaterials((Joint*)joint->mChild, &delegate);
	}
...
void BaseShape::calcWeightedMatrices()
{
	for (int i = 0; i < mEnvelopeCount; i++) {
		...
		for (int j = 0; j < mEnvelopeList[i].mIndexCount; j++) {
			...
			Matrix4f& mtx1 = weighted;
			Matrix4f& mtx2 = mAnimMatrices[mJointCount + i];

			f32* dst      = reinterpret_cast<f32*>(mtx2.mMtx);
			const f32* src = reinterpret_cast<const f32*>(mtx1.mMtx);
			for (int row = 0; row < 12; row++) {
				dst[row] += src[row] * weight;
			}
		}
	}
}
