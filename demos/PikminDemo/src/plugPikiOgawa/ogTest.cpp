...
void zen::OgTestScreen::update()
{
	...
	CardQuickInfo quickInfo {};
	int fileCheckRes = mFileChkSelMgr->update(mController, quickInfo);
...
void zen::OgTestScreen::draw(Graphics& gfx)
{
	...
	if (mActiveMode == TESTMODE_INACTIVE) {
		Matrix4f ortho;
		RectArea screenRect(0, 0, gfx.mScreenWidth, gfx.mScreenHeight);
		gfx.setOrthogonal(ortho.mMtx, screenRect);
	...
	if (mActiveMode == TESTMODE_Tutorial) {
		Matrix4f ortho;
		RectArea tutorialRect(0, 0, gfx.mScreenWidth, gfx.mScreenHeight);
		gfx.setOrthogonal(ortho.mMtx, tutorialRect);
		gfx.setColour(Colour(255, 255, 255, 255), true);
