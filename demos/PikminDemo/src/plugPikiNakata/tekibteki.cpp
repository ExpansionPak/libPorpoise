...
{
	InteractFlick flick(this, getParameterF(TPF_UpperFlickPower), 0.0f, FLICK_BACKWARDS_ANGLE);
	flickUpper(flick);
}
...
{
	InteractFlick flick(this, getParameterF(TPF_LowerFlickPower), getParameterF(TPF_LowerAttackPower), FLICK_BACKWARDS_ANGLE);
	flickLower(flick);
}
...
{
	TekiRecognitionCondition recognition(this);
	TekiStickingCondition stickingCond;
	TekiNotCondition notSticking(&stickingCond);
	TekiAndCondition recogAndNotStick(&recognition, &notSticking);

	TekiDistanceCondition distanceCond(this, getLowerRange());
	TekiAndCondition condition(&recogAndNotStick, &distanceCond);

	interactNavi(flick, condition);
	flick.mDamage = 0.0f;

	interactPiki(flick, condition);
...
void BTeki::collisionCallback(CollEvent& event)
{
	...
	if (collider->isAlive()) {
		TekiEvent tekiEvent(TekiEventType::Entity, static_cast<Teki*>(this), collider);
		eventPerformed(tekiEvent);
	}
}
...
{
	TekiEvent groundEvent(TekiEventType::Ground, static_cast<Teki*>(this));
	eventPerformed(groundEvent);
}
...
{
	TekiEvent wallEvent(TekiEventType::Wall, static_cast<Teki*>(this));
	eventPerformed(wallEvent);
}
 ...
 * Size:	0000F8
 */
void BTeki::drawRange(Graphics& gfx, Vector3f& centre, f32 range, const Colour& colour)
{
	Matrix4f mtx1;
	Matrix4f mtx2;
