...
{
	TekiVisibleCondition visible(&teki);
	Creature* naviPiki = teki.getClosestNaviPiki(visible, nullptr);
	if (!naviPiki) {
		return false;
	}
...
{
	TekiVisibleCondition visible(&teki);
	TekiNaviCondition naviCond;
	TekiAndCondition condition(&visible, &naviCond);
	Creature* navi = naviMgr->findClosest(teki.getPosition(), &condition);
	if (!navi) {
		return false;
	}
