...
bool TaiSwallowTurningAction::act(Teki& teki)
{
	...
	TekiRecognitionCondition recognition(&teki);
	if (!recognition.satisfy(target)) {
		PRINT_NAKATA("!TaiTurnAction::act:!condition.satisfy:%08x\n", &teki); // And again
		return true;
	}

	TekiLowerCondition lowerCond(&teki);
	TekiAndCondition condition(&recognition, &lowerCond);
	int pikiCount = teki.countPikis(condition);
	f32 linFuncValues[2];
...
{
	teki.flickUpper();
	InteractFlick flick(&teki, teki.getParameterF(TPF_LowerFlickPower), teki.getParameterF(TPF_LowerAttackPower), FLICK_BACKWARDS_ANGLE);
	TekiRecognitionCondition recog(&teki);
	TekiStickingCondition stickingCond;
	TekiNotCondition notSticking(&stickingCond);
	TekiAndCondition recogAndNotStick(&recog, &notSticking);

	TekiDistanceCondition distanceCond(&teki, teki.getLowerRange());
	TekiAngleCondition angleCond(&teki, teki.getParameterF(SWALLOWPF_FlickLowerAngle));
	TekiAndCondition distanceAndAngle(&distanceCond, &angleCond);

	TekiAndCondition condition(&recogAndNotStick, &distanceAndAngle);
	teki.interactNaviPiki(flick, condition);
}
...
{
	if (teki.getAnimationKeyOption(BTeki::ANIMATION_KEY_OPTION_ACTION_0)) {
		InteractFlick flick(&teki, teki.getParameterF(TPF_LowerFlickPower), teki.getParameterF(TPF_LowerAttackPower),
...
{
	TekiDistanceCondition noticeCond(&teki, teki.getParameterF(SWALLOWPF_NoticeDistance));
	Creature* naviPiki = teki.getClosestNaviPiki(noticeCond, nullptr);
	if (!naviPiki) {
		return false;
	}
