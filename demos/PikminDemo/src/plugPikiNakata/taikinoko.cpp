...
bool TaiKinokoDischargingSporesAction::act(Teki& teki)
{
	if (!teki.animationFinished()) {
		if (teki.getAnimationKeyOption(BTeki::ANIMATION_KEY_OPTION_ACTION_0)) {
			...
			InteractSpore spore(&teki);
			TekiRecognitionCondition recognition(&teki);
			TekiDistanceCondition distance(&teki, teki.getAttackRange());
			TekiAndCondition condition(&recognition, &distance);
			teki.interactNaviPiki(spore, condition);
