...
struct Vector3f {
	...
	void output(Vector3f& outVec) const { outVec.set(x, y, z); }
	...
	f32 DP(const Vector3f& other) const { return x * other.x + y * other.y + z * other.z; }
	f32 dot(const Vector3f& other) const { return x * other.x + y * other.y + z * other.z; }

	f32 distance(const Vector3f& to) const
	{
		Vector3f result;
		result.sub2(to, *this);
	...
	void cross(const Vector3f& vec1, const Vector3f& vec2)
	{
	...
	void add2(const Vector3f& a, const Vector3f& b) { set(a.x + b.x, a.y + b.y, a.z + b.z); }
	...
	void sub2(const Vector3f& a, const Vector3f& b) { set(a.x - b.x, a.y - b.y, a.z - b.z); }
	...
	void scale2(f32 scale, const Vector3f& vec)
	{
		x = scale * vec.x;
		y = scale * vec.y;
		z = scale * vec.z;
	...
	void bounce(const Vector3f& surface, f32 elasticity)
	{
	...
	void lerpTo(const Vector3f& other, f32 t, Vector3f& outVec) const
	{
	...
	void project(const Vector3f& dirToRemove)
	{
