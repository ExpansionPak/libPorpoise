 ...
 */
struct ayuID {
	inline ayuID(const char* id) { Set(id); }

	void Set(const char* id) { mID = *(const u32*)id; }
...
struct AyuStack {
	...
	// unused/inlined:
	void create(const char* name, int allocType, void* stackTop, int stackSize, bool isProtectionEnabled);
	void reset();
	void checkStack();
...
struct AyuHeap : public AyuStack {
	...
	void init(const char* name, int allocType, void* stackTop, int stackSize);
