...
struct Matrix4f {
	...
	void makeIdentity();
	void makeRotate(const Vector3f&, f32, f32);
	void makeRotate(const Vector3f&, f32);
	void multiply(Matrix4f&);
	void multiplyTo(const Matrix4f&, Matrix4f&) const;
	void makeSRT(const Vector3f&, const Vector3f&, const Vector3f&);
	void makeConcatSRT(Matrix4f*, Matrix4f&, const struct SRT&);
	void inverse(Matrix4f*);
	void scale(Vector3f&);
	void makeLookat(const Vector3f& cameraPos, const Vector3f& targetPos, Vector3f* optionalUp);
	void makeLookat(const Vector3f& cameraPos, const Vector3f& rightDir, const Vector3f& upDir, const Vector3f& backDir);
	void transposeTo(Matrix4f&);
	void makeVQS(const Vector3f&, struct Quat&, const Vector3f&);
