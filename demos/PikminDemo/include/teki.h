...
public:
	...
	inline Creature* getClosestNaviPiki(const Condition& condition, f32* dist)
	{
		return getClosestNaviPiki(const_cast<Condition&>(condition), dist);
	}
	bool attackTarget(Creature&, f32, f32, Condition&);
	inline bool attackTarget(Creature& creature, f32 p2, f32 p3, const Condition& condition)
	{
		return attackTarget(creature, p2, p3, const_cast<Condition&>(condition));
	}
	void outputHitCenter(Vector3f&);
	bool interactNaviPiki(Interaction&, Condition&);
	inline bool interactNaviPiki(Interaction& interaction, const Condition& condition)
	{
		return interactNaviPiki(interaction, const_cast<Condition&>(condition));
	}
	bool interactNavi(Interaction&, Condition&);
	inline bool interactNavi(Interaction& interaction, const Condition& condition)
	{
		return interactNavi(interaction, const_cast<Condition&>(condition));
	}
	bool interactPiki(Interaction&, Condition&);
	inline bool interactPiki(Interaction& interaction, const Condition& condition)
	{
		return interactPiki(interaction, const_cast<Condition&>(condition));
	}
	void flick();
	...
	inline bool checkNaviPiki(const Condition& condition) { return checkNaviPiki(const_cast<Condition&>(condition)); }
	int countPikis(Condition&);
	inline int countPikis(const Condition& condition) { return countPikis(const_cast<Condition&>(condition)); }
	...
	void drawRange(Graphics&, Vector3f&, f32, const struct Colour&);
	...
	static f32 calcDirection(const Vector3f& dir) { return NMathF::atan2(dir.x, dir.z); }

	void inputDirectionVector(const Vector3f& dir) { setDirection(calcDirection(dir)); }

	void inputVelocity(const Vector3f& vel) { mVelocity.input(vel); }
	void inputDrive(const Vector3f& drive) { mTargetVelocity.input(drive); }
...
public:
	...
	void initCylinderTYpePtclCallBack(Teki* p1, const Vector3f& p2, const Vector3f& p3, f32 p4, f32 p5, f32 p6, f32 p7,
	                                  TAIeffectAttackEventCallBack* cb)
	{
		Vector3f pos(p2);
		Vector3f dir(p3);
		initCylinderTYpePtclCallBack(p1, pos, dir, p4, p5, p6, p7, cb);
	}
	...
	void initConeTypePtclCallBack(Teki* p1, const Vector3f& p2, const Vector3f& p3, f32 p4, f32 p5, f32 p6, f32 p7,
	                              TAIeffectAttackEventCallBack* cb)
	{
		Vector3f pos(p2);
		Vector3f dir(p3);
		initConeTypePtclCallBack(p1, pos, dir, p4, p5, p6, p7, cb);
	}
