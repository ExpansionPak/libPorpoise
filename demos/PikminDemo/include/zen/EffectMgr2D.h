...
struct EffectRegister2D {
	...
	particleGenerator* create(particleManager& mgr, const Vector3f& pos, CallBack1<particleGenerator*>* cb1,
	                          CallBack2<particleGenerator*, particleMdl*>* cb2)
	{
		Vector3f copy(pos);
		return create(mgr, copy, cb1, cb2);
	}
...
struct EffectMgr2D {
	...
	inline particleGenerator* create(u32 id, const Vector3f& pos, zen::CallBack1<zen::particleGenerator*>* cb1,
	                                 zen::CallBack2<zen::particleGenerator*, zen::particleMdl*>* cb2)
	{
		Vector3f copy(pos);
		return create(id, copy, cb1, cb2);
	}
	void update();
	void draw(Graphics&);
	void killAll(bool);
