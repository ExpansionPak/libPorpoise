...
protected:
	virtual void flick(Teki& teki) // _1C
	{
		if (teki.mCurrentAnimEvent == KEY_Action0) {
			InteractFlick flick1(&teki, teki.getParameterF(TPF_UpperFlickPower), teki.getParameterF(TPF_UpperAttackPower),
			                     getFlickDirection(teki));
			InteractFlick flick2(&teki, teki.getParameterF(TPF_LowerFlickPower), teki.getParameterF(TPF_LowerAttackPower),
			                     getFlickDirection(teki));
			teki.flick(flick1, flick2);

			Vector3f dustPos(teki.getPosition());
			effectMgr->create(EffectMgr::EFF_BigDustRing, dustPos, nullptr, nullptr);
		}
	}
	...
	virtual bool act(Teki& teki) // _10
	{
		...
		if (TAIAreserveMotion::act(teki)) {
			...
			if (teki.getBiteSwitch() != false && teki.getCreaturePointer(2) == nullptr && nozzlePart != nullptr) {
				...
				CI_LOOP(pikiIter)
				{
					...
					if (pikiSlotSep.length() < teki.getParameterF(TAIbeatleFloatParms::Unk2)) {
						...
						Vector3f haloCentre(nozzlePart->mCentre);
						ptclGen = effectMgr->create(EffectMgr::EFF_Beatle_ShootRockHalo, haloCentre, nullptr, nullptr);
						if (ptclGen != nullptr) {
							Vector3f upVec(0.0f, 1.0f, 0.0f);
							ptclGen->setEmitDir(emitDir);
							ptclGen->setOrientedNormalVector(upVec);
						}

						Vector3f speckCentre(nozzlePart->mCentre);
						ptclGen = effectMgr->create(EffectMgr::EFF_Beatle_ShootRockSpecks, speckCentre, nullptr, nullptr);
						if (ptclGen != nullptr) {
							Vector3f upVec(0.0f, 1.0f, 0.0f);
							ptclGen->setEmitDir(emitDir);
							ptclGen->setOrientedNormalVector(upVec);
			...
			if (teki.mCurrentAnimEvent == KEY_Action2) {
				...
				if (kutiPart != nullptr) {
					...
					if (rock != nullptr) {
						...
						Vector3f rockCloudPos(vec1);
						ptclGen = effectMgr->create(EffectMgr::EFF_Beatle_RockClouds, rockCloudPos, nullptr, nullptr);
						if (ptclGen != nullptr) {
							ptclGen->setEmitDir(vec2);
						}

						Vector3f rockSprayPos(vec1);
						ptclGen = effectMgr->create(EffectMgr::EFF_Beatle_RockSpray, rockSprayPos, nullptr, nullptr);
						if (ptclGen != nullptr) {
							ptclGen->setEmitDir(vec2);
						}

						Vector3f rockBlastPos(vec1);
						ptclGen = effectMgr->create(EffectMgr::EFF_Beatle_RockBlast, rockBlastPos, nullptr, nullptr);
						if (ptclGen != nullptr) {
							ptclGen->setEmitDir(vec2);
						}
						...
						Vector3f flickPos(kutiPart->mCentre);
						ptclGen = effectMgr->create(EffectMgr::EFF_Beatle_Flick1, flickPos, nullptr, nullptr);
						if (ptclGen != nullptr) {
			...
			if (teki.getCreaturePointer(2) == nullptr && teki.mCurrentAnimEvent == KEY_LoopEnd) {
				mNextState = _0C;
				PaniMotionInfo motionInfo(-1, &teki);
				teki.mTekiAnimator->finishMotion(motionInfo);
			}
