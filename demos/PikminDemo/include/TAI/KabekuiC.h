...
protected:
	...
	virtual void attackEffect(Teki& teki) // _24
	{
		...
		if (teki.mCurrentAnimEvent == KEY_Action0) {
			CollPart* bitePart             = mouthSlot->getChildAt(mouthSlot->getChildCount() - 1);
			Vector3f biteCentre(bitePart->mCentre);
			zen::particleGenerator* generator
			    = effectMgr->create(EffectMgr::EFF_Kabekui_EatBridgeA, biteCentre, nullptr, nullptr);
			if (generator != nullptr) {
				Vector3f normal(1.0f, 0.0f, 0.0f);
				generator->setOrientedNormalVector(normal);
			}
		...
		if (teki.mCurrentAnimEvent != KEY_Action1 && teki.mCurrentAnimEvent == KEY_Action2) {
			...
			Vector3f biteCentre(bitePart->mCentre);
			zen::particleGenerator* generator
			    = effectMgr->create(EffectMgr::EFF_Kabekui_EatBridgeB, biteCentre, nullptr, nullptr);
			if (generator != nullptr) {
				Vector3f normal(1.0f, 0.0f, 0.0f);
				generator->setOrientedNormalVector(normal);
			}
