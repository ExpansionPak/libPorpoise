...
protected:
	virtual void flick(Teki& teki) // _1C
	{
		if (teki.mCurrentAnimEvent == KEY_Action0) {
			InteractFlick flick(&teki, teki.getParameterF(TPF_UpperFlickPower), teki.getParameterF(TPF_UpperAttackPower),
...
protected:
	...
	{
		TekiStickerCondition condition(&teki);
		int pikis = teki.countPikis(condition);
		if (pikis > 0) {
