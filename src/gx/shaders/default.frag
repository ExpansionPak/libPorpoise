...
uniform int u_ztex_enable;  /* GX_ZT_DISABLE/ADD/REPLACE */
uniform int u_ztex_op;
uniform float u_ztex_bias;  /* normalized bias added after z op */
...
uniform int u_chan_diff_fn;   /* GX_DF_* */
uniform int u_chan_attn_fn;   /* GX_AF_* */
uniform int u_lighting_enabled1;
uniform vec4 u_mat_color1;
uniform vec4 u_amb_color1;
uniform int u_chan_mat_src1;  /* 0=REG, 1=VTX */
uniform int u_chan_amb_src1;
uniform int u_chan_diff_fn1;  /* GX_DF_* */
uniform int u_chan_attn_fn1;  /* GX_AF_* */
...
uniform ivec3 u_tev_ksel[4];   /* x=kcolor_sel, y=kalpha_sel */
...
vec2 selectTexCoord(int src, vec2 tc0, vec2 tc1, vec2 tc2, vec2 tc3) {
    if (src == 1) return tc1;
    if (src == 2) return tc2;
    if (src == 3) return tc3;
    return tc0;
...
vec3 evalChanRGB(vec3 matC, vec3 ambC, int lightingEnabled, int lightMask, int diffFn, int attnFn, vec3 N) {
    ...
    for (int i = 0; i < 8; i++) {
        if ((lightMask & (1 << i)) != 0) {
            ...
            float ndotl = dot(N, L);
            float diff;
            if (diffFn == 0) {             /* GX_DF_NONE */
                diff = 1.0;
            } else if (diffFn == 1) {      /* GX_DF_SIGN */
                diff = ndotl;
            } else {                       /* GX_DF_CLAMP */
                diff = max(ndotl, 0.0);
            }
    ...
    /* GX-style channel output clamps after material modulation. */
    return clamp(matC * lightAccum, 0.0, 1.0);
}

vec4 rasterColorForChan(int chanSel, vec4 ras0, vec4 ras1) {
    if (chanSel == 1 || chanSel == 3 || chanSel == 5 || chanSel == 8) { /* COLOR1/ALPHA1/COLOR1A1/ALPHA_BUMPN */
        return ras1;
    }
    if (chanSel == 6 || chanSel == 255) { /* COLOR_ZERO / COLOR_NULL */
        return vec4(1.0);
    }
    return ras0; /* COLOR0 / ALPHA0 / COLOR0A0 / ALPHA_BUMP */
}

void main() {
    vec2 tc0 = v_texcoord0;
    vec2 tc1 = v_texcoord1;
    vec2 tc2 = v_texcoord2;
    vec2 tc3 = v_texcoord3;
    vec2 ind_prev_offset = vec2(0.0);

    /* Per-stage texcoord with indirect offset */
    vec2 stc0 = selectTexCoord(u_tev0_tc_src, tc0, tc1, tc2, tc3);
    if (u_num_ind_stages > 0 && u_tev0_ind_cfg.y != 0) {
        stc0 = applyIndirect(u_tev0_ind_cfg, u_tev0_ind_wrap, stc0, ind_prev_offset, tc0, tc1);
        ind_prev_offset = stc0 - selectTexCoord(u_tev0_tc_src, tc0, tc1, tc2, tc3);
    }
    vec2 stc1 = selectTexCoord(u_tev1_tc_src, tc0, tc1, tc2, tc3);
    if (u_num_ind_stages > 0 && u_num_tev_stages > 1 && u_tev1_ind_cfg.y != 0) {
        ...
        ind_prev_offset = stc1 - selectTexCoord(u_tev1_tc_src, tc0, tc1, tc2, tc3);
    }
    vec2 stc2 = selectTexCoord(u_tev2_tc_src, tc0, tc1, tc2, tc3);
    if (u_num_ind_stages > 0 && u_num_tev_stages > 2 && u_tev2_ind_cfg.y != 0) {
        ...
        ind_prev_offset = stc2 - selectTexCoord(u_tev2_tc_src, tc0, tc1, tc2, tc3);
    }
    vec2 stc3 = selectTexCoord(u_tev3_tc_src, tc0, tc1, tc2, tc3);
    if (u_num_ind_stages > 0 && u_num_tev_stages > 3 && u_tev3_ind_cfg.y != 0) {
        stc3 = applyIndirect(u_tev3_ind_cfg, u_tev3_ind_wrap, stc3, ind_prev_offset, tc0, tc1);
        ind_prev_offset = stc3 - selectTexCoord(u_tev3_tc_src, tc0, tc1, tc2, tc3);
    }
    ...
    if (u_num_chans == 0) {
        ...
        vec3 matC1 = (u_chan_mat_src1 != 0) ? v_color.rgb : u_mat_color1.rgb;
        vec3 ambC1 = (u_chan_amb_src1 != 0) ? v_color.rgb : u_amb_color1.rgb;
    ...
    /* TEV stage 0 */
    {
        ...
        vec4 sRas = applySwap(rasterColorForChan(u_tev0_color_chan, rasColor0, rasColor1),  u_swap_table[u_tev0_swap.x]);
    ...
    if (u_num_tev_stages > 1) {
        ...
        vec4 sRas = applySwap(rasterColorForChan(u_tev1_color_chan, rasColor0, rasColor1),  u_swap_table[u_tev1_swap.x]);
    ...
    if (u_num_tev_stages > 2) {
        ...
        vec4 sRas = applySwap(rasterColorForChan(u_tev2_color_chan, rasColor0, rasColor1),  u_swap_table[u_tev2_swap.x]);
    ...
    if (u_num_tev_stages > 3) {
        ...
        vec4 sRas = applySwap(rasterColorForChan(u_tev3_color_chan, rasColor0, rasColor1),  u_swap_table[u_tev3_swap.x]);
    ...
    if (u_ztex_enable != 0 && u_use_texture1 != 0) {
        float ztex = dot(texColor1.rgb, vec3(65536.0, 256.0, 1.0) / 16777215.0);
        float zout;
        if (u_ztex_op == 1) {          /* GX_ZT_ADD */
            zout = gl_FragCoord.z + ztex;
        } else if (u_ztex_op == 2) {   /* GX_ZT_REPLACE */
            zout = ztex;
        } else {
            zout = gl_FragCoord.z;
        }
        gl_FragDepth = clamp(zout + u_ztex_bias, 0.0, 1.0);
    }
